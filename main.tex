...
\begin{enumerate}
  \item Der Stromsee suggeriert, dass der "`Wasserstand"' im See steigen
    ...
    Verbrauch angepasst~\cite{wikipedia10kraftwerksmanagement}. Es ist
...
benutzen wir den "`Flukso"'~\cite{web:flukso}, um den den 
...
Kundenschnittstelle bieten \todo{Quelle: Positionspapier
Bundesnetzagentur}. Prinzipiell ist es natürlich möglich, die Smart
...
relevanten Informationen ausgesandt. Ein Mikrocontroller mit zugehöriger
...
Netzkompetenzzentrum} standardisiert wird. Aber selbst hier finden sich
Inkompatibilitäten in den Umsetzungen --- derzeit ist kein einheitlicher
Übertragungsstandard für die lokale Schnittstelle absehbar.
...
\begin{enumerate}
  ...
  \item Weiterhin ist es für Stromkunden hilfreich, den Effekt ihrer
    Handlungen unmittelbar zu sehen. Ein Wasserkocher 
...
zusätzlich auch einen "`Chumby"', vgl. Abbildung \todo{Bild}. Der Chumby
...
kostengünstig umzusetzen ist\todo{Referenz Octobus}.
\begin{figure}[htbp]
  \begin{center}
...
ein Relaus schalten, siehe Abb. \ref{fig:raven-adapter}.  Als
Betriebssystem benutzt OctoBus Contiki, \todo{Referenz}.
Alternativ ist auch mit Ethersex \todo{Referenz} eine etablierte Lösung
vorhanden. Da hochintegrierte Funkchipsätze sowie leistungsfähige
...
können~\footnote{Die Kalkukation ist auf der OctoBus-Seite
verfügbar}.
...
rentabelste Option. Dafür ist es notwendig, Haushaltsgeräte möglichst
dann zu betreiben, wenn die Photovoltaikanlage auf dem Dach gerade Strom
liefert. Für jede selbst erzeugte und selbst verbrauchte Kilowattstunde bekommt
der Anlagenbesitzer \euro XY \todo{Genaue Zahlen prüfen und einfügen.}.
Dazu kommen natürlich noch circa \euro $0,20$, die gespart werden, weil der Strom
nicht über den Hausanschluss von aussen eingekauft werden muss. Im
Vergleich dazu bekommt der Anlagenbesitzer \euro XY, wenn eine
Kilowattstunde selbst erzeugten Stroms in das Netz eingespeisst wird.
Unter dem Strich kann der Anlagenbesitzer also \euro XY mehr einnehmen,
wenn der Strom selbst verbraucht wird.  Dies macht auch
...
ist es aber für unsere Stromversorgung notwendig, die bestehenden Stromnetze
zu flexibilisieren und auf einen weiter steigenden Anteil von
erneuerbaren Energiequellen vorzubereiten. Damit dieser Wandel
...
allerdings zuwenig. Sowohl ökonomische, ökologische als auch
